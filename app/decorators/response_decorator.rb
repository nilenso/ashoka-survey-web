class ResponseDecorator < Draper::Base
  ...
  def input_tag_for(question, f)
    ...
    when 'PhotoQuestion'
      ...
      "#{(h.image_tag answer.photo_url(:medium), :class => 'medium' if answer.photo_url)} #{(f.input :photo, :as => :file, :required => question.mandatory, :label => question.content)}".html_safe
