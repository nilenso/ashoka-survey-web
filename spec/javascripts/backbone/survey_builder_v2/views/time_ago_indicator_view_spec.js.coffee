...
describe "TimeAgoIndicatorView", ->
  ...
  context "#start", ->
    it "changes the label text to 'Saved' immediately", ->
      ...
      $(".saving-indicator-time-ago-label").text().should.equal("Saved!")
    ...
    it "changes the label text to a relative time after 5 seconds", ->
      ...
      timeAgoIndicatorView.start()
      clock.tick(5000)
      $(".saving-indicator-time-ago-label").text().should.equal("Last saved a few seconds ago")

    it "keeps changing the relative time every five seconds", ->
      timeAgoIndicatorView = new SurveyBuilderV2.Views.TimeAgoIndicatorView
      timeAgoIndicatorView.start()
      clock.tick(5000)
      expect(-> $(".saving-indicator-time-ago-label").text()).to.change.when -> clock.tick(40000)
  ...
  context "#reset", ->
    it "resets the interval timer", ->
      ...
      expect(-> $(".saving-indicator-time-ago-label").text()).to.not.change.when -> clock.tick(5000)
