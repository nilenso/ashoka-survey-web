...
describe OrganizationDecorator do
  ...
  context "when fetching responses for an organization" do
    it "fetches only non-blank responses" do
      ...
      blank_response = FactoryGirl.create(:response, :survey => survey, :answers_present => false)
    ...
    it "fetches public responses" do
      ...
      response = FactoryGirl.create(:response, :answers_present, :survey => public_survey, :organization_id => nil)
      organization.response_count.should == 1
    end
  end
  ...
  context "when creating a chart of response counts grouped by month" do
    it "fetches the number of responses for an organization grouped by month of creation" do
      organization = OrganizationDecorator.decorate(FactoryGirl.build(:organization))
      survey = FactoryGirl.create(:survey, :organization_id => organization.id)
      Timecop.freeze("2013/04/15") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
      Timecop.freeze("2013/05/15") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
      Timecop.freeze("2013/05/16") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
    ...
    it "sorts the results by date" do
      organization = OrganizationDecorator.decorate(FactoryGirl.build(:organization))
      survey = FactoryGirl.create(:survey, :organization_id => organization.id)
      Timecop.freeze("2012/04/15") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
      Timecop.freeze("2013/05/15") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
      Timecop.freeze("2013/01/16") { FactoryGirl.create(:response, :answers_present, :survey_id => survey.id) }
    ...
    it "excludes blank responses" do
      organization = OrganizationDecorator.decorate(FactoryGirl.build(:organization))
      survey = FactoryGirl.create(:survey, :organization_id => organization.id)
      FactoryGirl.create(:response, :answers_present => false, :survey_id => survey.id)
  ...
  context "when calculating asset space" do
    it "calculates the cumulative file size for questions and answers" do
      organization = OrganizationDecorator.decorate(FactoryGirl.build(:organization))
      survey = FactoryGirl.create(:survey, :organization_id => organization.id)
      response = FactoryGirl.create(:response, :answers_present, :survey => survey, :organization_id => organization.id)
    ...
    it "doesn't include questions and answers from other organizations" do
      ...
      response = FactoryGirl.create(:response, :answers_present, :survey => survey, :organization_id => other_organization.id)
      question = FactoryGirl.create(:question, :survey => survey, :photo_file_size => 2048)
      answer = FactoryGirl.create(:answer, :response => response, :photo_file_size => 1024)
